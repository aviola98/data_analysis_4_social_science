...
banco_candidatos_resultados_RR <- banco_resultados_resumido %>%
  left_join(banco_candidatos_RR, by=c("SQ_CANDIDATO", "NM_CANDIDATO"))
...
```{r}
#corrigindo os NA em DS_GENERO  e NM_UE
banco_candidatos_resultados_RR <- banco_candidatos_resultados_RR %>%
  mutate(DS_GENERO=case_when(is.na(DS_GENERO)~ "FEMININO",
                             TRUE ~ DS_GENERO))
  
banco_candidatos_resultados_RR <- banco_candidatos_resultados_RR %>%
  mutate(NM_UE=case_when(is.na(NM_UE)~"RORAINÓPOLIS",
                         TRUE~NM_UE))
#7A
#graficos da 5B
#numero de candidatos para vereador por municipio (NM_UE)
banco_candidatos_resultados_RR %>%
  group_by(NM_UE,NM_CANDIDATO) %>%
  ggplot() +
